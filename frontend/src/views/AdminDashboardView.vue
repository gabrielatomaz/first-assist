<template>
  <div class="max-w-5xl mx-auto space-y-8 mt-8 animate-fadeIn">
    ...
    <div class="bg-bgCard p-6 rounded-2xl shadow border border-gray-100">
      <h2 class="text-3xl font-extrabold text-primaryNavy tracking-tight">System Administration</h2>
      <p class="text-sm text-gray-500 font-medium mt-1">Manage system configurations, user lists, and audit log histories</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-1 space-y-8">
        ...
        <div class="bg-bgCard p-6 rounded-2xl shadow border border-gray-100 space-y-4">
          <h3 class="text-base font-bold text-primaryNavy tracking-tight border-b pb-2">Event Registration</h3>
          ...
          <form @submit.prevent="createEvent" class="space-y-4">
            ...
            <div class="flex items-center space-x-2">
              ...
              <label for="isActive" class="text-xs text-gray-600 font-medium">Set as Active Event Context</label>
            </div>
        ...
        <!-- Team Registry -->
        <div class="bg-bgCard p-6 rounded-2xl shadow border border-gray-100 space-y-4">
          <h3 class="text-base font-bold text-primaryNavy tracking-tight border-b pb-2">Team Registration</h3>
          ...
          <form @submit.prevent="registerTeam" class="space-y-4">
            ...
            <button type="submit" class="w-full bg-primaryNavy text-white text-xs font-bold py-2 rounded shadow hover:bg-opacity-95">
              Register FRC Team
            </button>
          </form>
        </div>
      ...
      <div class="lg:col-span-2 space-y-8">
        <!-- Event list -->
        <div class="bg-bgCard p-6 rounded-2xl shadow border border-gray-100 space-y-4">
          <h3 class="text-base font-bold text-primaryNavy tracking-tight border-b pb-2">Registered Events</h3>
          
          <div class="max-h-48 overflow-y-auto space-y-2 pr-1">
            <div 
              v-for="ev in events" 
              :key="ev._id"
              class="p-3 rounded-lg border flex justify-between items-center text-xs"
              :class="ev.isActive ? 'border-primaryTeal bg-primaryTeal/5' : 'bg-white'"
            >
              <div>
                <span class="font-bold text-primaryNavy">{{ ev.name }} ({{ ev.code }})</span>
                <span class="block text-[10px] text-gray-400">{{ ev.location }}</span>
              </div>
              <button 
                v-if="!ev.isActive"
                @click="setActiveEvent(ev.code)"
                class="bg-white border text-primaryTeal px-2 py-1 rounded text-[10px] font-bold shadow hover:bg-gray-50"
              >
                Set Active
              </button>
              <span v-else class="text-[9px] bg-primaryTeal text-white font-extrabold px-2 py-0.5 rounded uppercase tracking-wider">
                Active
              </span>
            </div>
          </div>
        ...
        <div class="bg-bgCard p-6 rounded-2xl shadow border border-gray-100 space-y-4">
          <div class="flex justify-between items-center border-b pb-2">
            <h3 class="text-base font-bold text-primaryNavy tracking-tight">System Audit History Log</h3>
            <button @click="fetchAuditLogs" class="text-[10px] text-primaryTeal hover:underline font-bold uppercase tracking-wider">
              Refresh
            </button>
          </div>
          ...
          <div class="max-h-80 overflow-y-auto space-y-2 pr-1">
            ...
            <div 
              ...
              :key="log._id"
              class="p-3 rounded-lg border bg-white flex flex-col space-y-2 text-[11px] leading-relaxed"
            >
              <div class="flex justify-between items-center">
                <div class="flex items-center space-x-1.5">
                  <span class="font-bold text-primaryNavy">{{ log.userId?.name }}</span>
                  <span class="text-[9px] bg-gray-100 text-gray-600 font-extrabold uppercase font-mono px-1 py-0.5 rounded">
                    {{ log.userId?.role }}
                  </span>
                </div>
              ...
              <div>
                ...
                <span class="text-gray-700 font-medium">{{ log.details }}</span>
              </div>

              <div v-if="log.oldValue || log.newValue" class="grid grid-cols-2 gap-2 bg-gray-50/50 p-1.5 rounded border border-gray-100 text-[10px] font-mono">
                <div>
                  <span class="block text-[8px] font-extrabold text-gray-400 uppercase">Previous</span>
                  <span class="text-gray-500 truncate block">{{ log.oldValue || 'N/A' }}</span>
                </div>
                <div>
                  <span class="block text-[8px] font-extrabold text-gray-400 uppercase">New Value</span>
                  <span class="text-primaryNavy font-semibold truncate block">{{ log.newValue || 'N/A' }}</span>
                </div>
              </div>
            </div>
          </div>
