<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      ...
      <button @click="fetchIncidents" class="text-sm text-gray-500 hover:text-primaryTeal transition">&#8635; Refresh</button>
    </div>

    <div v-if="loading" class="text-center py-10">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-primaryTeal border-t-transparent"></div>
      <p class="text-gray-500 mt-3">Loading incidents...</p>
    </div>
...
const fetchIncidents = async () => {
  ...
  try {
    ...
    error.value = 'Backend unavailable. Showing mock data.'
